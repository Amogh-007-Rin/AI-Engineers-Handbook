...
Optimized tensor library for deep learning with dynamic computation graphs. This academy develops transferable judgment as well as tool fluency, from first setup through advanced and production use.

## Position in the handbook

- **Track:** Deep learning
- **Prerequisites:** Python, linear algebra, calculus, probability, and neural-network foundations.
- **Curriculum relationship:** Study the matching concept-first curriculum before or alongside this academy. This page owns PyTorch-specific workflows and does not replace underlying theory.

## Use it when

Use PyTorch when its abstractions match the problem, its ecosystem satisfies operational constraints, and a measured comparison supports the choice. Record the data size, latency, correctness, maintainability, team skill, licensing, and deployment assumptions behind the decision.

## Avoid it when

Do not select PyTorch only because it is popular or already listed here. Prefer a simpler standard-library solution, database, adjacent academy, managed service, or lower-level framework when it produces a safer and more maintainable system. The advanced assessment requires a comparison with at least one credible alternative.
...
3. Import or start PyTorch, print/report its version, and run one minimal operation.
...
- Tensor, module, parameter, and automatic-differentiation model.
- Input pipelines and training loops.
- Losses, optimizers, metrics, and checkpoints.

For each concept, predict behavior before execution, inspect types/shapes/state, add one normal assertion, and trigger one expected failure. Build a glossary mapping PyTorch terminology to the underlying concept.
...
- Custom models and reusable training components.
- Regularization and diagnostic workflows.
...
**Guided project:** Train, evaluate, checkpoint, and reproduce a small neural model with documented failure analysis.
...
- Compilation, mixed precision, and distributed execution.
- Profiling memory and throughput.
- Custom operations and framework internals.
...
**Independent project:** Implement and benchmark an advanced model, then export or serve it with operational tests.
...
- Export, serving, observability, and rollback.
- Accelerator, security, and dependency boundaries.

Create an operational checklist covering configuration, secrets, permissions, logs/metrics, failure recovery, dependency pinning, upgrade testing, artifact/data retention, and rollback. Include only items relevant to PyTorch, but justify omissions.
...
2. Understanding of PyTorch's mental model and appropriate API use.
...
Use the official PyTorch documentation, release notes, source repository, and primary papers as authoritative sources. Record exact links in project work. Never infer current APIs from this guide alone: verify version-sensitive behavior and update this page's review date when evidence changes.
